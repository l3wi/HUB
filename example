...
IOTA Hub is a tool for managing transfers from/to addresses in a simple and secure manner.  In this example, we create a new user with a deposit address.  This user receives iota then queries for the account balance.  

It was tested Ubuntu 18.04 LTS Server running Python 3.6.5 with these prerequisites:
```
backcall==0.1.0
decorator==4.3.0
...
Let’s step through the Python script.  First, the Hub is a gRPC server (https://grpc.io/docs/tutorials/) so we import grpc modules.  
```
import grpc
...
For demonstration purposes, we use an insecure channel of ‘localhost:50051’.  Put your IOTA Hub uri here.
...
We create a new user and generate a deposit address named, “Test”.  IncludeChecksum=True means that the address will contain the 81-tryte address plus the 9-tryte checksum for a total of 90-trytes.  
...
```
Next we get the account balance in iota.  This will be zero until someone transfers iota into your new deposit address.
...
```
Finally, we can see the transaction history for this address.
```
...
We can query IOTA Hub to view the new username, “Nelson”, and the account balance of 50i.  Notice, Nelson’s id is “9”.
